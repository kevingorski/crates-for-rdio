...
module.exports = function(app, rdio){
  ...
  function loadPlaybackToken(req, res, next) {
    if(req.session && req.session.playbackToken) {
      req.playbackToken = req.session.playbackToken;
    ...
    rdio.api(
      ...
      function(err, tokenResult) {
        ...
        req.session.playbackToken = req.playbackToken = JSON.parse(tokenResult).result;

        next();
      });
  }

  function renderCrate(req, res) {
    res.render('crate', {
      crate: req.crate,
  ...
  app.get('/crate/:id', loadCrate, loadAlbums, loadHeavyRotation, loadPlaybackToken, renderCrate);
